...
export class ReportesjrService {
   ...
   getById(id: number): Observable<any> {
      ...
      // obtenerPorCodigo(codigo: string): Observable<Reportes[]> {
      //    return this.http.get<Reportes[]>(`${baseUrl}/codigo/${codigo}`);
      // }
   ...
   uploadReporte(idrepoxopcion: number, nomrep: string, desrep: string, jrxml: File, jasper: File): Observable<any> {
   ...
   ejecutarReporteDB(dto: any): Observable<Blob> {
      return this.http.post(`${apiUrl}/jasperReports/descargar`, dto, { responseType: 'blob', });
   }

   // Actualiza solo los metadatos del reporte (sin archivos)
   actualizarMetadatos(id: number, datos: { idrepoxopcion: number; nomrep: string; desrep: string }): Observable<Reportesjr> {
