...
export class BeneficiarioService {
  ...
  constructor(private http: HttpClient) { }
   ...
   getBeneficiarios(nomben: String, codben: String, rucben: String, ciben: String) {
    return this.http.get<Beneficiario[]>(`${baseUrl}?nomben=${nomben}&codben=${codben}&rucben=${rucben}&ciben=${ciben}`);
 }
 ...
 findByGrupo(nomben: string, idgrupo: number) {
    return this.http.get<Beneficiario[]>( `${baseUrl}/nombreygrupo?nomben=${nomben}&idgrupo=${idgrupo}}` );
 }

 //Beneficiario por nombre para datalist
 findByNomben(nomben: string) {
    return this.http.get<Beneficiario[]>(`${baseUrl}/nomben?nomben=${nomben}`);
 }
 ...
 save(beneficiario: Beneficiario): Observable<Object> {
    return this.http.post(baseUrl, beneficiario);
 }

 getById(idbene: number) {
    return this.http.get<Beneficiario>(baseUrl + "/" + idbene);
 }

 updateBeneficiario(idbene: number, beneficiario: Beneficiario): Observable<Object> {
