...
@Component({
  ...
  styleUrl: './modi-cuenta.component.css',
})
export class ModiCuentaComponent implements OnInit {
    ...
    this.s_nivel.getAll().subscribe({
      next: (_niveles: any) => {
        ...
        this.f_cuenta.patchValue({
          nivel: _niveles[this._cuenta.nivel.idnivel - 1],
        });
      },
    });
  }
    ...
    this.s_cuentas.getByIdCuenta(idcuenta).subscribe({
      next: (datos: any) => {
        ...
        this.f_cuenta.patchValue({
          ...
          sigef: datos.sigef,
          resulcostos: datos.resulcostos,
          grubalances: datos.grubalances,
          gruefluefec: datos.gruefluefec,
          balancostos: datos.balancostos,
  ...
  save() {
    ...
    console.log(f);
    this.s_cuentas.updateCuenta(f).subscribe({
      next: (cuenta: any) => {
        console.log(cuenta);
